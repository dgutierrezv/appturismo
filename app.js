document.addEventListener('DOMContentLoaded', (event) => {
    ...
    // Usar las claves de API desde variables de entorno
    ...
    // Procesar la imagen con Google Cloud Vision API
    async function processImage(imageData) {
        try {
            ...
            if (data.responses[0].landmarkAnnotations && data.responses[0].landmarkAnnotations.length > 0) {
                ...
                const englishDescription = landmark.description;
                translateToSpanish(englishDescription);
            } else {
    ...
    // Traducir el nombre del lugar al español
    async function translateToSpanish(englishText) {
        try {
            ...
            if (data.data && data.data.translations && data.data.translations[0]) {
                const spanishText = data.data.translations[0].translatedText;
                ...
                throw new Error("Error en la estructura de la respuesta de la traducción");
            }
        } catch (error) {
